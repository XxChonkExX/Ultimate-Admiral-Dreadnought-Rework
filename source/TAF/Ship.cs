...
namespace TweaksAndFixes
{
    ...
    internal class Patch_Ship
    {
        ...
        [HarmonyPatch(nameof(Ship.Hover))]
        [HarmonyPrefix]
        internal static bool Prefix_Hover(Ship __instance, bool hover)
        {
            __instance.isHovered = hover;
            foreach (var p in __instance.hullAndParts)
                p.SetVisualMode(hover ? Part.VisualMode.Hover : Part.VisualMode.Normal);
    ...
    internal class Patch_VesselEntityFromStore
    {
        ...
        {
            // EPFM FIX: wrap the whole prefix in a guard so any null field in a
            // stock predefined design store (turretCalibers, mission, or
            // G.GameData.missions) cannot throw and kill the custom-battle
            // loading coroutine. On any failure, vanilla FromBaseStore proceeds.
            try
            {
            ...
            {
                // Swallow: TAF's optional grade-override data is not critical for
                // stock predefined ships. Let the vanilla load complete.
            }
        }
    }
}
